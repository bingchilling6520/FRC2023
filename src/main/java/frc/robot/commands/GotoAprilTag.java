...
public class GotoAprilTag extends CommandBase {
  ...
  public GotoAprilTag(int destTag) {
    // Use addRequirements() here to declare subsystem dependencies.
    addRequirements(VisionInstance);
    addRequirements(DriveBaseInstance);

    this.destTag = destTag;
    robotPose2d = VisionInstance.getPose2d();
  }
    ...
    if (photonRes.hasTargets()) {
      ...
      if (destTag == -1) {
        ...
        if (targetStream.count() <= 0) {
          ...
          DriveBaseInstance.drive(0, 0, 0); // Stop driving
          return;
        }
      ...
      DriveBaseInstance.drive(xSpeed, ySpeed, rSpeed);
    }
  }
  ...
  public void end(boolean interrupted) {
    DriveBaseInstance.drive(0, 0, 0); // Should stop immidiately
  }
